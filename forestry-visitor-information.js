(function ($, Drupal) {
    ...
    Drupal.behaviors.visitorInformation = {
        attach: function (context, settings) {
            ...
            // Secondary Nav bar.
            var secondaryNav = $('.secondary-nav .secondary-nav__items', context).first();
            // Check if there are any secondary nav links.
            if (viOverlay.length && secondaryNav && $('.secondary-nav__item', secondaryNav).length > 0) {
                // Add a visitor information shortcut.
                var viOverlayLink =
                    '<div class="secondary-nav__item">' +
                    '    <a href="#" class="vi-overlay-link">Visitor information</a>' +
                    '    <i class="icon icon_generic-chevron-right secondary-nav__item-icon" role="button"></i>' +
                    '</div>';
                secondaryNav.append(viOverlayLink);
                // On click/press open the VIO.
                $(secondaryNav).on('click', '.vi-overlay-link', openVIOverlay);
                $(secondaryNav).on('keypress', '.vi-overlay-link', function(e) {
                    if (e.which == 13) {
                        openVIOverlay();
                    }
                });
